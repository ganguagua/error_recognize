...
class BertModel(object):
  ...
  def __init__(self,
    ...
    input_ids = tf.placeholder_with_default(input_ids, tf.TensorShape([None, seq_length]))
    input_mask = tf.placeholder_with_default(input_mask, tf.TensorShape([None, seq_length]))
    token_type_ids = tf.placeholder_with_default(token_type_ids, tf.TensorShape([None, seq_length]))
    print(input_ids.name)
    print(input_mask.name)
    print(token_type_ids.name)
...
def transformer_model(input_tensor,
  ...
  input_shape = get_shape_list(input_tensor, expected_rank=3)
  batch_size = input_shape[0]
  tf.logging.info("batch_size: %s" %  str(input_tensor.shape))
  seq_length = input_shape[1]
  ...
  tf.logging.info("input shape: %s", str(input_shape))
